...
function DailyReport() {
  ...
  const { data: reportData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () => getDailyReport({ dateFrom: date, dateTo: date }),
    enabled: !!date,
  });
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-end gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-[#1E293B]">Daily Report</h1>
          <p className="mt-1 text-sm text-[#5B6E8C]">Task summary for a selected day</p>
        </div>
        <div className="ml-auto flex items-end gap-3">
          <div>
            <label className="block text-xs font-medium text-[#5B6E8C] mb-1">Report Date</label>
            <input
              type="date"
              value={date}
              max={today}
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              ...
              { label: 'Pending', value: reportData.summary?.pending ?? 0, color: 'text-amber-600' },
